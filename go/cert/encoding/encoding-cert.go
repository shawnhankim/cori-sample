package main

import (
	"encoding/base64"
	"io/ioutil"
	"os"
)

func main() {
	//keyStr := ""
	keyStr := "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
	savePEMKey("./key.pem", []byte(keyStr))
}

func savePEMKey(fileName string, key []byte) {
	outFile, _ := os.Create(fileName)
	defer outFile.Close()

	// var privateKey = &pem.Block{
	// 	Type:  "RSA PRIVATE KEY",
	// 	Bytes: key,
	// }
	//cert, _ := base64.StdEncoding.e.DecodeString(string(inSecret.Data["cert"]))
	encodeStr := base64.StdEncoding.EncodeToString(key)
	_ = ioutil.WriteFile(fileName, []byte(encodeStr), 0600)

	//_ = pem.Encode(outFile, privateKey)
}
